...
function Clear-Service {
    ...
    process {
        ...
        $_pg_host = $_profile -split "," | ForEach-Object {
            ...
            $_xpath = "//config/project/profile[@name='$_']/cfg[@namespace='Config.DB.db_kpt_cdc_event_pgsql_writer' and @key='server']"
        ...
        $_pg_port = $_profile -split "," | ForEach-Object {
            ...
            $_xpath = "//config/project/profile[@name='$_']/cfg[@namespace='Config.DB.db_kpt_cdc_event_pgsql_writer' and @key='port']"
            $_node = Select-Xml -XPath "$_xpath" -Path "$Workspace/config.xml"
            return $_node ? $_node.node.InnerXML : $null
        } | Where-Object { $_ } | Select-Object -First 1
        ...
        $_pg_database = $_profile -split "," | ForEach-Object {
            ...
            $_xpath = "//config/project/profile[@name='$_']/cfg[@namespace='Config.DB.db_kpt_cdc_event_pgsql_writer' and @key='database']"
            $_node = Select-Xml -XPath "$_xpath" -Path "$Workspace/config.xml"
            return $_node ? $_node.node.InnerXML : $null
        } | Where-Object { $_ } | Select-Object -First 1
        ...
        docker run -it --rm --name $_name `
            $_network `
            --mount "type=bind,source=$Workspace/clear.sql,destination=/pgconf/clear.sql" `
            --env MODE=sqlrunner `
