...
PATTERN_TOPIC_DIR=$(realpath "$PENTAHO_HOME/kpt-cdc-to-rdb/.pdi/lib/pentaho-big-data-kettle-plugins-kafka-${PDI_VERSION}") 
if [[ "$ENABLE_PATTERN_TOPIC" == "true" && -d "$PATTERN_TOPIC_DIR" ]] ; then
    function_link_libs "$PATTERN_TOPIC_DIR" "$PENTAHO_HOME/data-integration/system/karaf/deploy"
    echo "pattern topic enabled!"
else
    function_unlink_libs "$PATTERN_TOPIC_DIR" "$PENTAHO_HOME/data-integration/system/karaf/deploy"
    echo "pattern topic disbaled!"
fi
