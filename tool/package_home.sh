...
# TIME 2022-04-26
# FILE package_home
# DESC package kettle home
# SYNTAX package_home [repository_name_regex [kpt_workspace_path [pdi_engine_path]]]
...
# tip info
tip_repository_name_regex_input="Need input 'repository_name_regex':"
tip_repository_name_regex_miss="Missing param 'repository_name_regex' at position 1."
tip_repository_name_regex_wrong="Wrong param 'repository_name_regex' at position 1."
...
while [[ -z "$repository_name_regex" ]]; do
    if [[ $interactive -eq 1 ]]; then 
        read -p "$tip_repository_name_regex_input" repository_name_regex
	else 
        echo "$tip_repository_name_regex_miss"
        exit 1
    fi
done
while [[ -f "$kpt_workspace_path/$repository_name_regex" ]]; then
    if [[ $interactive -eq 1 ]]; then 
        echo "exist $kpt_workspace_path/$repository_name_regex"
        read -p "$tip_repository_name_regex_input" repository_name_regex
    else
        echo "$tip_repository_name_regex_wrong"
        exit 1
    fi
fi
...
export KPT_KETTLE_FILE="$current_script_dir/Deploy/PackageZipDeploy4KettleHome.kjb"
export KPT_KETTLE_PARAM_rNameRegex="$repository_name_regex"
